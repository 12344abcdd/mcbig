...
public class MobAppearanceParticle extends Particle {
    ...
    @Override
    public void renderCustom(PoseStack p_383017_, MultiBufferSource p_382805_, Camera p_383007_, float p_383153_) {
        float f = ((float)this.age + p_383153_) / (float)this.lifetime;
        float f1 = 0.05F + 0.5F * Mth.sin(f * (float) Math.PI);
        int i = ARGB.colorFromFloat(f1, 1.0F, 1.0F, 1.0F);
        p_383017_.pushPose();
        p_383017_.mulPose(p_383007_.rotation());
        p_383017_.mulPose(Axis.XP.rotationDegrees(60.0F - 150.0F * f));
        float f2 = 0.42553192F;
        p_383017_.scale(0.42553192F, -0.42553192F, -0.42553192F);
        p_383017_.translate(0.0F, -0.56F, 3.5F);
        VertexConsumer vertexconsumer = p_382805_.getBuffer(this.renderType);
        this.model.renderToBuffer(p_383017_, vertexconsumer, 15728880, OverlayTexture.NO_OVERLAY, i);
        p_383017_.popPose();
    }

...
public class EntityRenderDispatcher implements ResourceManagerReloadListener {
    ...
    public EntityRenderDispatcher(
        ...
        ItemModelResolver p_388181_,
        ItemRenderer p_234581_,
        MapRenderer p_361209_,
        BlockRenderDispatcher p_234582_,
        Font p_234583_,
        Options p_234584_,
        Supplier<EntityModelSet> p_387861_,
        EquipmentAssetManager p_386713_
    ) {
    ...
    public <E extends Entity> void render(
        E p_114385_, double p_114386_, double p_114387_, double p_114388_, float p_114389_, PoseStack p_114391_, MultiBufferSource p_114392_, int p_114393_
    ) {
    ...
    private <E extends Entity, S extends EntityRenderState> void render(
        ...
        float p_364461_,
        PoseStack p_363537_,
        MultiBufferSource p_364290_,
        int p_363520_,
        EntityRenderer<? super E, S> p_360327_
    ) {
        try {
            S s = p_360327_.createRenderState(p_365426_, p_364461_);
            Vec3 vec3 = p_360327_.getRenderOffset(s);
            double d3 = p_364222_ + vec3.x();
            double d0 = p_360564_ + vec3.y();
            double d1 = p_365367_ + vec3.z();
            p_363537_.pushPose();
            p_363537_.translate(d3, d0, d1);
            p_360327_.render(s, p_363537_, p_364290_, p_363520_);
            if (s.displayFireAnimation) {
                this.renderFlame(p_363537_, p_364290_, s, Mth.rotationAroundAxis(Mth.Y_AXIS, this.cameraOrientation, new Quaternionf()));
            }

            if (p_365426_ instanceof Player) {
                p_363537_.translate(-vec3.x(), -vec3.y(), -vec3.z());
            }

            if (this.options.entityShadows().get() && this.shouldRenderShadow && !s.isInvisible) {
                float f = p_360327_.getShadowRadius(s);
                if (f > 0.0F) {
                    double d2 = s.distanceToCameraSq;
                    float f1 = (float)((1.0 - d2 / 256.0) * (double)p_360327_.getShadowStrength(s));
                    if (f1 > 0.0F) {
                        renderShadow(p_363537_, p_364290_, s, f1, p_364461_, this.level, Math.min(f, 32.0F));
                    }
                }
            }
            ...
            if (this.renderHitBoxes && !s.isInvisible && !Minecraft.getInstance().showOnlyReducedInfo()) {
                renderHitbox(p_363537_, p_364290_.getBuffer(RenderType.lines()), p_365426_, p_364461_, 1.0F, 1.0F, 1.0F);
            }
            ...
            crashreportcategory1.setDetail("Assigned renderer", p_360327_);
            crashreportcategory1.setDetail("Location", CrashReportCategory.formatLocation(this.level, p_364222_, p_360564_, p_365367_));
            crashreportcategory1.setDetail("Delta", p_364461_);
    ...
    private static void renderServerSideHitbox(PoseStack p_353069_, Entity p_353061_, MultiBufferSource p_353074_) {
        ...
        if (entity == null) {
            ...
            ShapeRenderer.renderVector(p_353069_, p_353074_.getBuffer(RenderType.lines()), new Vector3f(), entity.getDeltaMovement(), -256);
            p_353069_.popPose();
        }
    }
    ...
    private static void renderHitbox(
        ...
        ShapeRenderer.renderVector(
            p_114442_, p_114443_, new Vector3f(0.0F, p_114444_.getEyeHeight(), 0.0F), p_114444_.getViewVector(p_114445_).scale(2.0), -16776961
        );
    }
    ...
    private static void renderShadow(
        PoseStack p_114458_, MultiBufferSource p_114459_, EntityRenderState p_365403_, float p_114461_, float p_114462_, LevelReader p_114463_, float p_114464_
    ) {
        float f = Math.min(p_114461_ / 0.5F, p_114464_);
        int i = Mth.floor(p_365403_.x - (double)p_114464_);
        int j = Mth.floor(p_365403_.x + (double)p_114464_);
        int k = Mth.floor(p_365403_.y - (double)f);
        int l = Mth.floor(p_365403_.y);
        int i1 = Mth.floor(p_365403_.z - (double)p_114464_);
        int j1 = Mth.floor(p_365403_.z + (double)p_114464_);
        ...
        for (int k1 = i1; k1 <= j1; k1++) {
            for (int l1 = i; l1 <= j; l1++) {
                ...
                for (int i2 = k; i2 <= l; i2++) {
                    ...
                    renderBlockShadow(
                        posestack$pose, vertexconsumer, chunkaccess, p_114463_, blockpos$mutableblockpos, p_365403_.x, p_365403_.y, p_365403_.z, p_114464_, f1
                    );
                }
            }
        }
            ...
            if (blockstate.isCollisionShapeFullBlock(p_277501_, blockpos)) {
                ...
                if (!voxelshape.isEmpty()) {
                    ...
                    if (f1 >= 0.0F) {
                        ...
                        double d0 = (double)p_277911_.getBigX().doubleValue() + aabb.minX;
                        double d1 = (double)p_277911_.getBigX().doubleValue() + aabb.maxX;
                        double d2 = (double)p_277911_.getBigY().doubleValue() + aabb.minY;
                        double d3 = (double)p_277911_.getBigZ().doubleValue() + aabb.minZ;
                        double d4 = (double)p_277911_.getBigZ().doubleValue() + aabb.maxZ;
    ...
    public double distanceToSqr(Entity p_114472_) {
        return this.camera.getPosition().distanceToSqr(p_114472_.bigPosition());
    }

...
public class ItemInHandRenderer {
    ...
    public ItemInHandRenderer(Minecraft p_234241_, EntityRenderDispatcher p_234242_, ItemRenderer p_234243_, ItemModelResolver p_390372_) {
        this.minecraft = p_234241_;
        this.entityRenderDispatcher = p_234242_;
        this.itemRenderer = p_234243_;
        this.itemModelResolver = p_390372_;
    }
    ...
    private void applyEatTransform(PoseStack p_109331_, float p_109332_, HumanoidArm p_109333_, ItemStack p_109334_, Player p_346361_) {
        float f = (float)p_346361_.getUseItemRemainingTicks() - p_109332_ + 1.0F;
        float f1 = f / (float)p_109334_.getUseDuration(p_346361_);
        if (f1 < 0.8F) {
    ...
    private void applyBrushTransform(PoseStack p_273513_, float p_273245_, HumanoidArm p_273726_, ItemStack p_272809_, Player p_345294_, float p_273333_) {
        ...
        float f = (float)(p_345294_.getUseItemRemainingTicks() % 10);
    ...
    private void swingArm(float p_389706_, float p_389465_, PoseStack p_389695_, int p_389683_, HumanoidArm p_389577_) {
        float f = -0.4F * Mth.sin(Mth.sqrt(p_389706_) * (float) Math.PI);
        float f1 = 0.2F * Mth.sin(Mth.sqrt(p_389706_) * (float) (Math.PI * 2));
        float f2 = -0.2F * Mth.sin(p_389706_ * (float) Math.PI);
        p_389695_.translate((float)p_389683_ * f, f1, f2);
        this.applyItemArmTransform(p_389695_, p_389577_, p_389465_);
        this.applyItemArmAttackTransform(p_389695_, p_389577_, p_389706_);
    }

    private boolean shouldInstantlyReplaceVisibleItem(ItemStack p_390460_, ItemStack p_390457_) {
        return ItemStack.matches(p_390460_, p_390457_) ? true : !this.itemModelResolver.shouldPlaySwapAnimation(p_390457_);
    }

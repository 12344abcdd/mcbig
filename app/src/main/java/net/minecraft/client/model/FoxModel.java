...
public class FoxModel extends EntityModel<FoxRenderState> {
    ...
    public void setupAnim(FoxRenderState p_361178_) {
        super.setupAnim(p_361178_);
        float f = p_361178_.walkAnimationSpeed;
        float f1 = p_361178_.walkAnimationPos;
        ...
        this.head.zRot = p_361178_.headRollAngle;
        this.rightHindLeg.visible = true;
        this.leftHindLeg.visible = true;
        this.rightFrontLeg.visible = true;
        this.leftFrontLeg.visible = true;
        float f2 = p_361178_.ageScale;
        if (p_361178_.isCrouching) {
            this.body.xRot += 0.10471976F;
            float f3 = p_361178_.crouchAmount;
            this.body.y += f3 * f2;
            this.head.y += f3 * f2;
        } else if (p_361178_.isSleeping) {
        ...
        if (!p_361178_.isSleeping && !p_361178_.isFaceplanted && !p_361178_.isCrouching) {
            this.head.xRot = p_361178_.xRot * (float) (Math.PI / 180.0);
            this.head.yRot = p_361178_.yRot * (float) (Math.PI / 180.0);
        }

        if (p_361178_.isSleeping) {
            this.head.xRot = 0.0F;
            this.head.yRot = (float) (-Math.PI * 2.0 / 3.0);
            this.head.zRot = Mth.cos(p_361178_.ageInTicks * 0.027F) / 22.0F;
        }

        if (p_361178_.isCrouching) {
            float f4 = Mth.cos(p_361178_.ageInTicks) * 0.01F;
            this.body.yRot = f4;

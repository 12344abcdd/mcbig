...
public class ShulkerModel extends EntityModel<ShulkerRenderState> {
    ...
    public void setupAnim(ShulkerRenderState p_362026_) {
        super.setupAnim(p_362026_);
        float f = (0.5F + p_362026_.peekAmount) * (float) Math.PI;
        float f1 = -1.0F + Mth.sin(f);
        float f2 = 0.0F;
        if (f > (float) Math.PI) {
            f2 = Mth.sin(p_362026_.ageInTicks * 0.1F) * 0.7F;
        }
        ...
        this.head.xRot = p_362026_.xRot * (float) (Math.PI / 180.0);
        this.head.yRot = (p_362026_.yHeadRot - 180.0F - p_362026_.yBodyRot) * (float) (Math.PI / 180.0);
    }
}

...
public class IronGolemModel extends EntityModel<IronGolemRenderState> {
    ...
    public void setupAnim(IronGolemRenderState p_360733_) {
        super.setupAnim(p_360733_);
        float f = p_360733_.attackTicksRemaining;
        float f1 = p_360733_.walkAnimationSpeed;
        float f2 = p_360733_.walkAnimationPos;
        if (f > 0.0F) {
            this.rightArm.xRot = -2.0F + 1.5F * Mth.triangleWave(f, 10.0F);
            this.leftArm.xRot = -2.0F + 1.5F * Mth.triangleWave(f, 10.0F);
        } else {
            int i = p_360733_.offerFlowerTick;
            if (i > 0) {
        ...
        this.head.yRot = p_360733_.yRot * (float) (Math.PI / 180.0);
        this.head.xRot = p_360733_.xRot * (float) (Math.PI / 180.0);

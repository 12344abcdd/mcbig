...
public class ParrotModel extends EntityModel<ParrotRenderState> {
    ...
    public void setupAnim(ParrotRenderState p_361630_) {
        super.setupAnim(p_361630_);
        this.prepare(p_361630_.pose);
        this.head.xRot = p_361630_.xRot * (float) (Math.PI / 180.0);
        this.head.yRot = p_361630_.yRot * (float) (Math.PI / 180.0);
        switch (p_361630_.pose) {
            case STANDING:
                this.leftLeg.xRot = this.leftLeg.xRot + Mth.cos(p_361630_.walkAnimationPos * 0.6662F) * 1.4F * p_361630_.walkAnimationSpeed;
                this.rightLeg.xRot = this.rightLeg.xRot + Mth.cos(p_361630_.walkAnimationPos * 0.6662F + (float) Math.PI) * 1.4F * p_361630_.walkAnimationSpeed;
            case FLYING:
            case ON_SHOULDER:
            default:
                float f2 = p_361630_.flapAngle * 0.3F;
                this.head.y += f2;
                this.tail.xRot = this.tail.xRot + Mth.cos(p_361630_.walkAnimationPos * 0.6662F) * 0.3F * p_361630_.walkAnimationSpeed;
                this.tail.y += f2;
                this.body.y += f2;
                this.leftWing.zRot = -0.0873F - p_361630_.flapAngle;
                this.leftWing.y += f2;
                this.rightWing.zRot = 0.0873F + p_361630_.flapAngle;
                this.rightWing.y += f2;
                this.leftLeg.y += f2;
                this.rightLeg.y += f2;
            case SITTING:
                break;
            case PARTY:
                float f = Mth.cos(p_361630_.ageInTicks);
                float f1 = Mth.sin(p_361630_.ageInTicks);
                this.head.x += f;
                this.head.y += f1;
                this.head.xRot = 0.0F;
                this.head.yRot = 0.0F;
                this.head.zRot = Mth.sin(p_361630_.ageInTicks) * 0.4F;
                this.body.x += f;
                this.body.y += f1;
                this.leftWing.zRot = -0.0873F - p_361630_.flapAngle;
                this.leftWing.x += f;
    ...
    private void prepare(ParrotModel.Pose p_361450_) {
        switch (p_361450_) {
            case FLYING:
    ...
    public static ParrotModel.Pose getPose(Parrot p_361411_) {
        if (p_361411_.isPartyParrot()) {
            return ParrotModel.Pose.PARTY;
        } else if (p_361411_.isInSittingPose()) {
            return ParrotModel.Pose.SITTING;
        } else {
            return p_361411_.isFlying() ? ParrotModel.Pose.FLYING : ParrotModel.Pose.STANDING;
        }
    }

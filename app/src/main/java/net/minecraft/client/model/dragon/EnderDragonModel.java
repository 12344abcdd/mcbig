...
public class EnderDragonModel extends EntityModel<EnderDragonRenderState> {
    ...
    public EnderDragonModel(ModelPart p_361417_) {
        ...
        for (int i = 0; i < this.neckParts.length; i++) {
            this.neckParts[i] = p_361417_.getChild(neckName(i));
        }

        for (int j = 0; j < this.tailParts.length; j++) {
            this.tailParts[j] = p_361417_.getChild(tailName(j));
        }
    ...
    public void setupAnim(EnderDragonRenderState p_360968_) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            modelpart.xRot = f8
                + p_360968_.getHeadPartYOffset(i, dragonflighthistory$sample, dragonflighthistory$sample1) * (float) (Math.PI / 180.0) * 1.5F * 5.0F;
        ...
        this.head.xRot = Mth.wrapDegrees(p_360968_.getHeadPartYOffset(6, dragonflighthistory$sample, dragonflighthistory$sample2))
            * (float) (Math.PI / 180.0)
            * 1.5F
            * 5.0F;
    ...
    private void poseLimbs(
        float p_363142_, ModelPart p_362750_, ModelPart p_364979_, ModelPart p_362786_, ModelPart p_363025_, ModelPart p_360492_, ModelPart p_363358_
    ) {

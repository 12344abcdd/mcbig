...
public class ProfilerPieChart {
    ...
    public void render(GuiGraphics p_363707_) {
        if (this.profilerPieChartResults != null) {
            ...
            p_363707_.drawSpecial(p_371353_ -> {
                double d0 = 0.0;

                for (ResultField resultfield2 : list) {
                    int i3 = Mth.floor(resultfield2.percentage / 4.0) + 1;
                    VertexConsumer vertexconsumer = p_371353_.getBuffer(RenderType.debugTriangleFan());
                    int j3 = ARGB.opaque(resultfield2.getColor());
                    int k3 = ARGB.multiply(j3, -8355712);
                    PoseStack.Pose posestack$pose = p_363707_.pose().last();
                    ...
                    vertexconsumer = p_371353_.getBuffer(RenderType.debugQuads());
            ...
            int i2 = 16777215;
            int j2 = l1 - 62;
            p_363707_.drawString(this.font, s1, j, j2, 16777215);
            s1 = decimalformat.format(resultfield.globalPercentage) + "%";
            p_363707_.drawString(this.font, s1, k - this.font.width(s1), j2, 16777215);
            ...
            for (int k2 = 0; k2 < list.size(); k2++) {
                ...
                int l2 = j1 + k2 * 9;
                p_363707_.drawString(this.font, s2, j, l2, resultfield1.getColor());
                s2 = decimalformat.format(resultfield1.percentage) + "%";
                p_363707_.drawString(this.font, s2, k - 50 - this.font.width(s2), l2, resultfield1.getColor());
                s2 = decimalformat.format(resultfield1.globalPercentage) + "%";
                p_363707_.drawString(this.font, s2, k - this.font.width(s2), l2, resultfield1.getColor());
            }
        }
    }
        ...
        if (this.profilerPieChartResults != null) {
            ...
            if (!list.isEmpty()) {
                ...
                if (p_364522_ == 0) {
                    ...
                    p_364522_--;
                    if (p_364522_ < list.size() && !"unspecified".equals(list.get(p_364522_).name)) {
                        if (!this.profilerTreePath.isEmpty()) {
                            this.profilerTreePath = this.profilerTreePath + "\u001e";
                        }

                        this.profilerTreePath = this.profilerTreePath + list.get(p_364522_).name;
                    }
                }
            }
        }

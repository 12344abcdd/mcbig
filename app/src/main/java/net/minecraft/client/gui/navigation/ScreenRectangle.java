...
public record ScreenRectangle(ScreenPosition position, int width, int height) {
    ...
    public boolean containsPoint(int p_332787_, int p_332731_) {
        return p_332787_ >= this.left() && p_332787_ < this.right() && p_332731_ >= this.top() && p_332731_ < this.bottom();
    }

    public ScreenRectangle transformAxisAligned(Matrix4f p_382785_) {
        if (MatrixUtil.isIdentity(p_382785_)) {
            return this;
        } else {
            Vector3f vector3f = p_382785_.transformPosition((float)this.left(), (float)this.top(), 0.0F, new Vector3f());
            Vector3f vector3f1 = p_382785_.transformPosition((float)this.right(), (float)this.bottom(), 0.0F, new Vector3f());

...
public class RecipeButton extends AbstractWidget {
    ...
    public void init(RecipeCollection p_100480_, boolean p_360894_, RecipeBookPage p_100481_, ContextMap p_381053_) {
        ...
        List<RecipeDisplayEntry> list = p_100480_.getSelectedRecipes(
            p_360894_ ? RecipeCollection.CraftableStatus.CRAFTABLE : RecipeCollection.CraftableStatus.ANY
        );
        this.selectedEntries = list.stream().map(p_380798_ -> new RecipeButton.ResolvedEntry(p_380798_.id(), p_380798_.resultItems(p_381053_))).toList();
    ...
    private static boolean allRecipesHaveSameResultDisplay(List<RecipeButton.ResolvedEntry> p_389698_) {
        Iterator<ItemStack> iterator = p_389698_.stream().flatMap(p_389328_ -> p_389328_.displayItems().stream()).iterator();
        if (!iterator.hasNext()) {
            return true;
        } else {
    ...
    public List<Component> getTooltipText(ItemStack p_379408_) {
        List<Component> list = new ArrayList<>(Screen.getTooltipFromItem(Minecraft.getInstance(), p_379408_));
        if (this.hasMultipleRecipes()) {

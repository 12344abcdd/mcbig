...
public interface Registry<T> extends Keyable, HolderLookup.RegistryLookup<T>, IdMap<T> {
    ...
    @Nullable
    T getValue(@Nullable ResourceKey<T> p_368650_);

    @Nullable
    T getValue(@Nullable ResourceLocation p_368511_);
    ...
    default T getValueOrThrow(ResourceKey<T> p_368664_) {
        T t = this.getValue(p_368664_);
        if (t == null) {
            throw new IllegalStateException("Missing key in " + this.key() + ": " + p_368664_);
        } else {
            return t;
        }
    }
    ...
    Optional<Holder.Reference<T>> get(int p_368617_);

    Optional<Holder.Reference<T>> get(ResourceLocation p_123002_);
    ...
    Registry.PendingTags<T> prepareTagReload(TagLoader.LoadResult<T> p_363095_);

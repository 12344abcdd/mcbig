        ...
        if (player != null && !this.mayPlace(player, direction, itemstack, blockpos1)) {
            ...
            if (hangingentity.survives()) {
                if (!level.isClientSide) {
                    ...
                    level.gameEvent(player, GameEvent.ENTITY_PLACE, hangingentity.bigPosition());
        ...
        if (holderlookup$provider != null && this.type == EntityType.PAINTING) {
            ...
            if (!customdata.isEmpty()) {
                customdata.read(holderlookup$provider.createSerializationContext(NbtOps.INSTANCE), Painting.VARIANT_MAP_CODEC)
                    .result()
                    .ifPresentOrElse(p_360027_ -> {
                        p_360027_.value().title().ifPresent(p_270630_::add);
                        p_360027_.value().author().ifPresent(p_270630_::add);
                        p_270630_.add(Component.translatable("painting.dimensions", p_360027_.value().width(), p_360027_.value().height()));

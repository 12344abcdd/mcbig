...
public class FireworkRocketEntity extends Projectile implements ItemSupplier {
    ...
    public void tick() {
        ...
        if (this.isAttachedToEntity()) {
            if (this.attachedToEntity == null) {
                this.entityData.get(DATA_ATTACHED_TO_TARGET).ifPresent(p_390276_ -> {
                    Entity entity = this.level().getEntity(p_390276_);
    ...
    private void dealExplosionDamage(ServerLevel p_376666_) {
        ...
        if (f > 0.0F) {
            if (this.attachedToEntity != null) {
                this.attachedToEntity.hurtServer(p_376666_, this.damageSources().fireworks(this, this.getOwner()), 5.0F + (float)(list.size() * 2));
            }
            ...
            for (LivingEntity livingentity : this.level().getEntitiesOfClass(LivingEntity.class, this.getBoundingBox().inflate(5.0))) {
                if (livingentity != this.attachedToEntity && !(this.distanceToSqr(livingentity) > 25.0)) {
                    ...
                    for (int i = 0; i < 2; i++) {
                        ...
                        HitResult hitresult = this.level().clip(new ClipContext(vec3.toBig(), vec31.toBig(), ClipContext.Block.COLLIDER, ClipContext.Fluid.NONE, this));
                    ...
                    if (flag) {
                        ...
                        livingentity.hurtServer(p_376666_, this.damageSources().fireworks(this, this.getOwner()), f1);
                    }
                }
            }
        }

...
public abstract class AbstractWindCharge extends AbstractHurtingProjectile implements ItemSupplier {
    ...
    @Override
    protected AABB makeBoundingBox(Vec3 p_390351_) {
        float f = this.getType().getDimensions().width() / 2.0F;
        float f1 = this.getType().getDimensions().height();
        float f2 = 0.15F;
        return new AABB(
            p_390351_.x - (double)f,
            p_390351_.y - 0.15F,
            p_390351_.z - (double)f,
            p_390351_.x + (double)f,
            p_390351_.y - 0.15F + (double)f1,
            p_390351_.z + (double)f
        );
    }
    ...
    protected abstract void explode(me.alphamode.mcbig.core.BigVec3 p_352265_);

    @Override
    protected void onHitBlock(BlockHitResult p_325933_) {
        super.onHitBlock(p_325933_);
        if (!this.level().isClientSide) {
            Vec3i vec3i = p_325933_.getDirection().getUnitVec3i();
            Vec3 vec3 = Vec3.atLowerCornerOf(vec3i).multiply(0.25, 0.25, 0.25);
            me.alphamode.mcbig.core.BigVec3 vec31 = p_325933_.getLocation().add(vec3);
            this.explode(vec31);
            this.discard();
        }
    }

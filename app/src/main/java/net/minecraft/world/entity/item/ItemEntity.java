...
public class ItemEntity extends Entity implements TraceableEntity {
    ...
    private void setFluidMovement(double p_376453_) {
        Vec3 vec3 = this.getDeltaMovement();
        this.setDeltaMovement(vec3.x * p_376453_, vec3.y + (double)(vec3.y < 0.06F ? 5.0E-4F : 0.0F), vec3.z * p_376453_);
    }
    ...
    public final boolean hurtServer(ServerLevel p_376942_, DamageSource p_376452_, float p_376562_) {
        if (this.isInvulnerableToBase(p_376452_)) {
            return false;
        } else if (!p_376942_.getGameRules().getBoolean(GameRules.RULE_MOBGRIEFING) && p_376452_.getEntity() instanceof Mob) {
            return false;

...
public class PiglinAi {
    ...
    private static void initIdleActivity(Brain<Piglin> p_34892_) {
        p_34892_.addActivity(
            ...
            ImmutableList.of(
                ...
                StartAttacking.<Piglin>create((p_390258_, p_390259_) -> p_390259_.isAdult(), PiglinAi::findNearestValidAttackTarget),
    ...
    private static void initCelebrateActivity(Brain<Piglin> p_34921_) {
        p_34921_.addActivityAndRemoveMemoryWhenStopped(
            ...
            ImmutableList.<net.minecraft.world.entity.ai.behavior.BehaviorControl<? super Piglin>>of(
                avoidRepellent(),
                SetEntityLookTarget.create(PiglinAi::isPlayerHoldingLovedItem, 14.0F),
                StartAttacking.<Piglin>create((p_390260_, p_390261_) -> p_390261_.isAdult(), PiglinAi::findNearestValidAttackTarget),
    ...
    protected static void pickUpItem(ServerLevel p_376319_, Piglin p_34844_, ItemEntity p_34845_) {
        ...
        if (isLovedItem(itemstack)) {
            ...
            holdInOffhand(p_376319_, p_34844_, itemstack);
            admireGoldItem(p_34844_);
        } else if (isFood(itemstack) && !hasEatenRecently(p_34844_)) {
            eat(p_34844_);
        } else {
            boolean flag = !p_34844_.equipItemIfPossible(p_376319_, itemstack).equals(ItemStack.EMPTY);
            if (!flag) {
    ...
    private static void holdInOffhand(ServerLevel p_376251_, Piglin p_34933_, ItemStack p_34934_) {
        if (isHoldingItemInOffHand(p_34933_)) {
            p_34933_.spawnAtLocation(p_376251_, p_34933_.getItemInHand(InteractionHand.OFF_HAND));
        }
    ...
    protected static void stopHoldingOffHandItem(ServerLevel p_376454_, Piglin p_34868_, boolean p_34869_) {
        ...
        if (p_34868_.isAdult()) {
            ...
            if (p_34869_ && flag) {
                ...
                boolean flag1 = !p_34868_.equipItemIfPossible(p_376454_, itemstack).isEmpty();
                if (!flag1) {
                    putInInventory(p_34868_, itemstack);
                }
            }
        } else {
            boolean flag2 = !p_34868_.equipItemIfPossible(p_376454_, itemstack).isEmpty();
            if (!flag2) {
    ...
    protected static void cancelAdmiring(ServerLevel p_376381_, Piglin p_34928_) {
        if (isAdmiringItem(p_34928_) && !p_34928_.getOffhandItem().isEmpty()) {
            p_34928_.spawnAtLocation(p_376381_, p_34928_.getOffhandItem());
    ...
    private static boolean isNearestValidAttackTarget(ServerLevel p_376103_, Piglin p_34901_, LivingEntity p_34902_) {
        return findNearestValidAttackTarget(p_376103_, p_34901_).filter(p_34887_ -> p_34887_ == p_34902_).isPresent();
    }
    ...
    private static Optional<? extends LivingEntity> findNearestValidAttackTarget(ServerLevel p_376590_, Piglin p_35001_) {
        ...
        if (isNearZombified(p_35001_)) {
            ...
            if (optional.isPresent() && Sensor.isEntityAttackableIgnoringLineOfSight(p_376590_, p_35001_, optional.get())) {
                return optional;
            } else {
                ...
                if (optional3.isPresent()) {
                    ...
                    return optional2.isPresent() && Sensor.isEntityAttackable(p_376590_, p_35001_, optional2.get()) ? optional2 : Optional.empty();
                }
            }
        }
    }

    public static void angerNearbyPiglins(ServerLevel p_376662_, Player p_34874_, boolean p_34875_) {
        List<Piglin> list = p_34874_.level().getEntitiesOfClass(Piglin.class, p_34874_.getBoundingBox().inflate(16.0));
        list.stream().filter(PiglinAi::isIdle).filter(p_34881_ -> !p_34875_ || BehaviorUtils.canSee(p_34881_, p_34874_)).forEach(p_375921_ -> {
            if (p_376662_.getGameRules().getBoolean(GameRules.RULE_UNIVERSAL_ANGER)) {
                setAngerTargetToNearestTargetablePlayerIfFound(p_376662_, p_375921_, p_34874_);
            } else {
                setAngerTarget(p_376662_, p_375921_, p_34874_);
            }
        });
    }

    public static InteractionResult mobInteract(ServerLevel p_376885_, Piglin p_34847_, Player p_34848_, InteractionHand p_34849_) {
        ...
        if (canAdmire(p_34847_, itemstack)) {
            ...
            holdInOffhand(p_376885_, p_34847_, itemstack1);
    ...
    protected static void wasHurtBy(ServerLevel p_376829_, Piglin p_34838_, LivingEntity p_34839_) {
        if (!(p_34839_ instanceof Piglin)) {
            if (isHoldingItemInOffHand(p_34838_)) {
                stopHoldingOffHandItem(p_376829_, p_34838_, false);
            }
            ...
            getAvoidTarget(p_34838_).ifPresent(p_375916_ -> {
                if (p_375916_.getType() != p_34839_.getType()) {
                    brain.eraseMemory(MemoryModuleType.AVOID_TARGET);
                }
            });
            if (p_34838_.isBaby()) {
                brain.setMemoryWithExpiry(MemoryModuleType.AVOID_TARGET, p_34839_, 100L);
                if (Sensor.isEntityAttackableIgnoringLineOfSight(p_376829_, p_34838_, p_34839_)) {
                    broadcastAngerTarget(p_376829_, p_34838_, p_34839_);
                }
                ...
                maybeRetaliate(p_376829_, p_34838_, p_34839_);
            }
        }
    }

    protected static void maybeRetaliate(ServerLevel p_376349_, AbstractPiglin p_34827_, LivingEntity p_34828_) {
        if (!p_34827_.getBrain().isActive(Activity.AVOID)) {
            if (Sensor.isEntityAttackableIgnoringLineOfSight(p_376349_, p_34827_, p_34828_)) {
                if (!BehaviorUtils.isOtherTargetMuchFurtherAwayThanCurrentAttackTarget(p_34827_, p_34828_, 4.0)) {
                    if (p_34828_.getType() == EntityType.PLAYER && p_376349_.getGameRules().getBoolean(GameRules.RULE_UNIVERSAL_ANGER)) {
                        setAngerTargetToNearestTargetablePlayerIfFound(p_376349_, p_34827_, p_34828_);
                        broadcastUniversalAnger(p_376349_, p_34827_);
                    } else {
                        setAngerTarget(p_376349_, p_34827_, p_34828_);
                        broadcastAngerTarget(p_376349_, p_34827_, p_34828_);
                    }
                }
            }
        }
    ...
    private static BehaviorControl<LivingEntity> babySometimesRideBabyHoglin() {
        ...
        return CopyMemoryWithExpiry.create(
            p_390257_ -> p_390257_.isBaby() && setentitylooktargetsometimes$ticker.tickDownAndCheck(p_390257_.level().random),
    ...
    protected static void broadcastAngerTarget(ServerLevel p_376810_, AbstractPiglin p_376255_, LivingEntity p_376115_) {
        getAdultPiglins(p_376255_).forEach(p_375906_ -> {
            if (p_376115_.getType() != EntityType.HOGLIN || p_375906_.canHunt() && ((Hoglin)p_376115_).canBeHunted()) {
                setAngerTargetIfCloserThanCurrent(p_376810_, p_375906_, p_376115_);
            }
        });
    }

    protected static void broadcastUniversalAnger(ServerLevel p_376377_, AbstractPiglin p_34896_) {
        getAdultPiglins(p_34896_)
            .forEach(p_375913_ -> getNearestVisibleTargetablePlayer(p_375913_).ifPresent(p_375924_ -> setAngerTarget(p_376377_, p_375913_, p_375924_)));
    }

    protected static void setAngerTarget(ServerLevel p_376847_, AbstractPiglin p_34925_, LivingEntity p_34926_) {
        if (Sensor.isEntityAttackableIgnoringLineOfSight(p_376847_, p_34925_, p_34926_)) {
            ...
            if (p_34926_.getType() == EntityType.PLAYER && p_376847_.getGameRules().getBoolean(GameRules.RULE_UNIVERSAL_ANGER)) {
                p_34925_.getBrain().setMemoryWithExpiry(MemoryModuleType.UNIVERSAL_ANGER, true, 600L);
            }
        }
    }

    private static void setAngerTargetToNearestTargetablePlayerIfFound(ServerLevel p_376569_, AbstractPiglin p_34945_, LivingEntity p_34946_) {
        Optional<Player> optional = getNearestVisibleTargetablePlayer(p_34945_);
        if (optional.isPresent()) {
            setAngerTarget(p_376569_, p_34945_, optional.get());
        } else {
            setAngerTarget(p_376569_, p_34945_, p_34946_);
        }
    }

    private static void setAngerTargetIfCloserThanCurrent(ServerLevel p_376637_, AbstractPiglin p_34963_, LivingEntity p_34964_) {
        ...
        if (!optional.isPresent() || optional.get() != livingentity) {
            setAngerTarget(p_376637_, p_34963_, livingentity);
        }
    }

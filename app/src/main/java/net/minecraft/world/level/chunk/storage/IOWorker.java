...
public class IOWorker implements ChunkScanAccess, AutoCloseable {
    ...
    protected IOWorker(RegionStorageInfo p_325953_, Path p_196930_, boolean p_196931_) {
        ...
        this.consecutiveExecutor = new PriorityConsecutiveExecutor(IOWorker.Priority.values().length, Util.ioPool(), "IOWorker-" + p_325953_.type());
    }
    ...
    public CompletableFuture<Void> store(ChunkPos p_360728_, Supplier<CompoundTag> p_361805_) {
        return this.<CompletableFuture<Void>>submitTask(() -> {
            CompoundTag compoundtag = p_361805_.get();
            IOWorker.PendingStore ioworker$pendingstore = this.pendingWrites.computeIfAbsent(p_360728_, p_223488_ -> new IOWorker.PendingStore(compoundtag));
    ...
    public CompletableFuture<Void> synchronize(boolean p_182499_) {
        CompletableFuture<Void> completablefuture = this.<CompletableFuture<Void>>submitTask(
            ...
            .thenCompose(Function.identity());
        return p_182499_ ? completablefuture.thenCompose(p_371174_ -> this.submitThrowingTask(() -> {
                try {
                    this.storage.flush();
                    return null;
                } catch (Exception exception) {
    ...
    private <T> CompletableFuture<T> submitThrowingTask(IOWorker.ThrowingSupplier<T> p_371938_) {

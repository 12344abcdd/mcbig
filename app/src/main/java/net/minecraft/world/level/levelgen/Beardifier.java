...
public class Beardifier implements DensityFunctions.BeardifierOrMarker {
    ...
    public static Beardifier forStructuresInChunk(StructureManager p_223938_, ChunkPos p_223939_) {
        me.alphamode.mcbig.math.BigInteger i = p_223939_.getMinBlockX();
        me.alphamode.mcbig.math.BigInteger j = p_223939_.getMinBlockZ();
                        ...
                        if (structurepiece.isCloseToChunk(p_223939_, 12)) {
                            if (structurepiece instanceof PoolElementStructurePiece) {
                                ...
                                for (JigsawJunction jigsawjunction : poolelementstructurepiece.getJunctions()) {
                                    ...
                                    if (k.compareTo(i.subtract(me.alphamode.mcbig.core.BigConstants.Ints.TWELVE)) > 0 && l.compareTo(j.subtract(me.alphamode.mcbig.core.BigConstants.Ints.TWELVE)) > 0 && k.compareTo(i.add(15 + 12)) < 0 && l.compareTo(j.add(15 + 12)) < 0) {
    ...
    public double compute(DensityFunction.FunctionContext p_208200_) {
        ...
        while (this.pieceIterator.hasNext()) {
            ...
            int i1 = Math.max(0, Math.max(boundingbox.minX().subtract(i).intValue(), i.subtract(boundingbox.maxX()).intValue()));
            int j1 = Math.max(0, Math.max(boundingbox.minZ().subtract(k).intValue(), k.subtract(boundingbox.maxZ()).intValue()));
            ...
            int i2 = switch (beardifier$rigid.terrainAdjustment()) {
                ...
                case ENCAPSULATE -> Math.max(0, Math.max(boundingbox.minY().intValue() - j, j - boundingbox.maxY().intValue()));
            };

...
public interface CollisionGetter extends BlockGetter {
    ...
    default boolean noCollision(@Nullable Entity p_364564_, AABB p_364942_, boolean p_362912_) {
        for (VoxelShape voxelshape : p_362912_ ? this.getBlockAndLiquidCollisions(p_364564_, p_364942_) : this.getBlockCollisions(p_364564_, p_364942_)) {
        ...
        if (!this.getEntityCollisions(p_364564_, p_364942_).isEmpty()) {
            return false;
        } else if (p_364564_ == null) {
            return true;
        } else {
            VoxelShape voxelshape1 = this.borderCollision(p_364564_, p_364942_);
            return voxelshape1 == null || !Shapes.joinIsNotEmpty(voxelshape1, Shapes.create(p_364942_), BooleanOp.AND);
        }
    }
    ...
    default Iterable<VoxelShape> getBlockCollisions(@Nullable Entity p_186435_, AABB p_186436_) {
        return () -> new BlockCollisions<>(this, p_186435_, p_186436_, false, (p_360113_, p_360114_) -> p_360114_);
    }

    default Iterable<VoxelShape> getBlockAndLiquidCollisions(@Nullable Entity p_364621_, AABB p_365424_) {
        return () -> new BlockCollisions<>(this, CollisionContext.of(p_364621_, true), p_365424_, false, (p_286215_, p_286216_) -> p_286216_);
    }
    ...
    default BlockHitResult clipIncludingBorder(ClipContext p_365129_) {
        BlockHitResult blockhitresult = this.clip(p_365129_);
        WorldBorder worldborder = this.getWorldBorder();
        if (worldborder.isWithinBounds(p_365129_.getFrom()) && !worldborder.isWithinBounds(blockhitresult.getLocation())) {
            me.alphamode.mcbig.core.BigVec3 vec3 = blockhitresult.getLocation().subtract(p_365129_.getFrom());
            Direction direction = Direction.getApproximateNearest(vec3.x.doubleValue(), vec3.y, vec3.z.doubleValue());
            me.alphamode.mcbig.core.BigVec3 vec31 = worldborder.clampVec3ToBound(blockhitresult.getLocation());

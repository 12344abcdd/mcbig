...
public class SpreadPlayersCommand {
                                            ...
                                            Commands.argument("respectTeams", BoolArgumentType.bool())
                                                .then(
                                                    Commands.argument("targets", EntityArgument.entities())
                                                        .executes(
                                                            p_382678_ -> spreadPlayers(
                                                                    p_382678_.getSource(),
                                                                    Vec2Argument.getVec2(p_382678_, "center"),
                                                                    FloatArgumentType.getFloat(p_382678_, "spreadDistance"),
                                                                    FloatArgumentType.getFloat(p_382678_, "maxRange"),
                                                                    p_382678_.getSource().getLevel().getMaxY() + 1,
                                                                    BoolArgumentType.getBool(p_382678_, "respectTeams"),
                                                                    EntityArgument.getEntities(p_382678_, "targets")
                                                                )
                                                        )
                                                )
                                        )
    ...
    private static double setPlayerPositions(
        ...
        for (Entity entity : p_138730_) {
            ...
            entity.teleportTo(
                p_138731_,
                Mth.bigFloor(spreadplayerscommand$position.x).toBigDecimal().add(me.alphamode.mcbig.core.BigConstants.AABB),
                (double)spreadplayerscommand$position.getSpawnY(p_138731_, p_138733_),
                Mth.bigFloor(spreadplayerscommand$position.z).toBigDecimal().add(me.alphamode.mcbig.core.BigConstants.AABB),
                Set.of(),
                entity.getYRot(),
                entity.getXRot(),
                true

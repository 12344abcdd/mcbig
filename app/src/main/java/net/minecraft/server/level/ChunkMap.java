...
public class ChunkMap extends ChunkStorage implements ChunkHolder.PlayerProvider, GeneratingChunkMap {
    ...
    private final it.unimi.dsi.fastutil.objects.Object2LongMap<ChunkPos> nextChunkSaveTime = new it.unimi.dsi.fastutil.objects.Object2LongOpenHashMap<>();
    private final it.unimi.dsi.fastutil.objects.ObjectSet<ChunkPos> chunksToEagerlySave = new it.unimi.dsi.fastutil.objects.ObjectLinkedOpenHashSet<>();
    ...
    public ChunkMap(
        ...
        this.lightEngine = new ThreadedLevelLightEngine(
            p_214842_, this, this.level.dimensionType().hasSkyLight(), consecutiveexecutor, this.lightTaskDispatcher
        );
    ...
    private void setChunkUnsaved(ChunkPos p_381702_) {
        this.chunksToEagerlySave.add(p_381702_);
    }
        ...
        if (p_282030_ == 0) {
            ...
            for (int j = -p_282030_; j <= p_282030_; j++) {
                for (int k = -p_282030_; k <= p_282030_; k++) {
                    ...
                    ChunkPos i1 = new ChunkPos(chunkpos.x.add(k), chunkpos.z.add(j));
        ...
        if (!ChunkLevel.isLoaded(p_140180_) && !ChunkLevel.isLoaded(p_140178_)) {
            ...
            if (ChunkLevel.isLoaded(p_140178_) && p_140179_ == null) {
                ...
                if (p_140179_ != null) {
                    ...
                    p_140179_ = new ChunkHolder(p_140177_, p_140178_, this.level, this.lightEngine, this::onLevelChange, this);
                }
    ...
    private void processUnloads(BooleanSupplier p_140354_) {
        for (it.unimi.dsi.fastutil.objects.ObjectIterator<ChunkPos> longiterator = this.toDrop.iterator(); longiterator.hasNext(); longiterator.remove()) {
    ...
    private void saveChunksEagerly(BooleanSupplier p_381708_) {
        long i = Util.getMillis();
        int j = 0;
        it.unimi.dsi.fastutil.objects.ObjectIterator<ChunkPos> longiterator = this.chunksToEagerlySave.iterator();

        while (j < 20 && this.activeChunkWrites.get() < 128 && p_381708_.getAsBoolean() && longiterator.hasNext()) {
            ChunkPos k = longiterator.next();
        ...
        completablefuture.thenRunAsync(() -> {
            ...
            if (completablefuture1 != completablefuture) {
                ...
                if (this.pendingUnloads.remove(p_140182_, p_140183_) && chunkaccess != null) {
                    ...
                    this.nextChunkSaveTime.removeLong(chunkaccess.getPos());
                }
            }
        }, this.unloadQueue::add).whenComplete((p_381764_, p_381765_) -> {
            if (p_381765_ != null) {
                LOGGER.error("Failed to save chunk {}", p_140183_.getPos(), p_381765_);
            }
        });
    }
    ...
    private CompletableFuture<ChunkAccess> scheduleChunkLoad(ChunkPos p_140418_) {
        ...
        return completablefuture.<Object, Optional<SerializableChunkData>>thenCombine(
            ...
            .exceptionallyAsync(p_329919_ -> this.handleChunkLoadFailure(p_329919_, p_140418_), this.mainThreadExecutor);
    }
    ...
    private byte markPosition(ChunkPos p_140230_, ChunkType p_332120_) {
        return this.chunkTypeCache.put(p_140230_, (byte)(p_332120_ == ChunkType.PROTOCHUNK ? -1 : 1));
    }

    @Override
    public GenerationChunkHolder acquireGeneration(ChunkPos p_347661_) {
...
//                crashreportcategory.setDetail("Position hash", ChunkPos.asLong(chunkpos.x, chunkpos.z));
    ...
    private void runGenerationTask(ChunkGenerationTask p_347721_) {
        ...
        }, generationchunkholder.getPos(), generationchunkholder::getQueueLevel);
    }
    ...
    public CompletableFuture<ChunkResult<LevelChunk>> prepareTickingChunk(ChunkHolder p_143054_) {
        ...
        CompletableFuture<ChunkResult<LevelChunk>> completablefuture1 = completablefuture.thenApplyAsync(p_359611_ -> p_359611_.map(p_381756_ -> {
                ...
                if (completablefuture2.isDone()) {
                    ...
                    completablefuture2.thenAcceptAsync(p_381759_ -> this.onChunkReadyToSend(p_143054_, levelchunk), this.mainThreadExecutor);
                }
    ...
    private boolean isExistingChunkFull(ChunkPos p_140426_) {
        byte b0 = this.chunkTypeCache.get(p_140426_);
        if (b0 != 0) {
            return b0 == 1;
        } else {
...
//            ChunkPos i = entry.getKey();
    ...
    void forEachSpawnCandidateChunk(Consumer<ChunkHolder> p_365506_) {
        it.unimi.dsi.fastutil.objects.ObjectIterator<ChunkPos> longiterator = this.distanceManager.getSpawnCandidateChunks();
        ...
        while (longiterator.hasNext()) {
            ...
            if (chunkholder != null && this.anyPlayerCloseEnoughForSpawningInternal(chunkholder.getPos())) {
                p_365506_.accept(chunkholder);
            }
        }
    }

    boolean anyPlayerCloseEnoughForSpawning(ChunkPos p_183880_) {
        return !this.distanceManager.hasPlayersNearby(p_183880_) ? false : this.anyPlayerCloseEnoughForSpawningInternal(p_183880_);
    }

    private boolean anyPlayerCloseEnoughForSpawningInternal(ChunkPos p_363995_) {
        for (ServerPlayer serverplayer : this.playerMap.getAllPlayers()) {
            if (this.playerIsCloseEnoughForSpawning(serverplayer, p_363995_)) {
                return true;
            }
        }

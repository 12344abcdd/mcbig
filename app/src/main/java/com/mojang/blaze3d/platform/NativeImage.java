...
public final class NativeImage implements AutoCloseable {
    ...
    private static NativeImage putAndRead(ByteBuffer p_389700_, byte[] p_389448_) throws IOException {
        p_389700_.put(p_389448_);
        p_389700_.rewind();
        return read(p_389700_);
    }
    ...
    private int getPixelABGR(int p_364787_, int p_362067_) {
        ...
        } else if (this.isOutsideBounds(p_364787_, p_362067_)) {
            throw new IllegalArgumentException(
                String.format(Locale.ROOT, "(%s, %s) outside of image bounds (%s, %s)", p_364787_, p_362067_, this.width, this.height)
            );
        } else {
            this.checkAllocated();
            long i = ((long)p_364787_ + (long)p_362067_ * (long)this.width) * 4L;
            return MemoryUtil.memGetInt(this.pixels + i);
        }
    }

    public int getPixel(int p_361017_, int p_364883_) {
        return ARGB.fromABGR(this.getPixelABGR(p_361017_, p_364883_));
    }

    private void setPixelABGR(int p_364345_, int p_360552_, int p_362876_) {
        if (this.format != NativeImage.Format.RGBA) {
            throw new IllegalArgumentException(String.format(Locale.ROOT, "setPixelRGBA only works on RGBA images; have %s", this.format));
        } else if (this.isOutsideBounds(p_364345_, p_360552_)) {
            throw new IllegalArgumentException(
                String.format(Locale.ROOT, "(%s, %s) outside of image bounds (%s, %s)", p_364345_, p_360552_, this.width, this.height)
            );
        } else {
            this.checkAllocated();
            long i = ((long)p_364345_ + (long)p_360552_ * (long)this.width) * 4L;
            MemoryUtil.memPutInt(this.pixels + i, p_362876_);
        }
    }

    public void setPixel(int p_362575_, int p_360685_, int p_361735_) {
        this.setPixelABGR(p_362575_, p_360685_, ARGB.toABGR(p_361735_));
    }
    ...
    public void upload(int p_85014_, int p_85015_, int p_85016_, int p_85017_, int p_85018_, int p_85019_, int p_85020_, boolean p_85021_) {
        if (!RenderSystem.isOnRenderThreadOrInit()) {
            RenderSystem.recordRenderCall(() -> this._upload(p_85014_, p_85015_, p_85016_, p_85017_, p_85018_, p_85019_, p_85020_, p_85021_));
        } else {
            this._upload(p_85014_, p_85015_, p_85016_, p_85017_, p_85018_, p_85019_, p_85020_, p_85021_);
        }
    }

...
public class Uniform extends AbstractUniform implements AutoCloseable {
    ...
    public void setFromConfig(ShaderProgramConfig.Uniform p_380976_) {
        this.setFromConfig(p_380976_.values(), p_380976_.count());
    }

    public void setFromConfig(List<Float> p_381052_, int p_381131_) {
        float[] afloat = new float[Math.max(p_381131_, 16)];
        if (p_381052_.size() == 1) {
            Arrays.fill(afloat, p_381052_.getFirst().floatValue());
        } else {
            for (int i = 0; i < p_381052_.size(); i++) {
                afloat[i] = p_381052_.get(i);
            }
        }
        ...
        if (this.type <= 3) {
            ...
            this.set(Arrays.copyOfRange(afloat, 0, p_381131_));
        }
    }
